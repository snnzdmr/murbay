...
void APP_Handle(){
	...
	while(1){
		...
		pressedKey = p_KeypadObj->Scan(&longPres);
		if(pressedKey > -1 && longPres == 0){                         		 // SHORT PRESS ? 
			if(ptrWorkVariable->qytFlag == true){															// IS QYT OPEN  ? 
				weight = setPrice((char)pressedKey,p_LcdObj_S1,&m_Param);       // Acik ise girilen sayiyi 1 .ekrana yaz (qyt kg yerine el ile kg girme anlamina geliyor)
			}else{                                                            //  QYT kapali ise normal calis  
				price = setPrice((char)pressedKey,p_LcdObj_S2,&m_Param);        
			}
			switch(pressedKey){
				case 'p':                                                       // PLU re-call 
				...
				case 'h': 																												// CH 
					...
					break;
				case 'q':																													// QYT 
					if(ptrWorkVariable->qytFlag ==  true){													/// QYT TOGGLE
						ptrWorkVariable->qytFlag =false;
					}
					else{
						ptrWorkVariable->qytFlag =true;
					}
					break;
			}
		}
		if(pressedKey > -1 && longPres == 1){                            // LONG PRESS ? 
			longPres=0;	
...
bool APP_Show_Weight(float *_weight,MENU_Params *p_MenuParam){
	...
	uint8_t buffer[7]="";
	int len = 0;
	if(ptrWorkVariable->qytFlag == true){ 
		return false;
	}
